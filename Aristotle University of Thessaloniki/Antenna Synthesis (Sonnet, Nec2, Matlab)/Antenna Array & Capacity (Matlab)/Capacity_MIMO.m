function C_SNR = Capacity_MIMO(NT,NR,dT,dR,DD,Ns,Nc)
...
% General parameters
lamda = 1.0; k0 = 2*pi/lamda; 
LT = lamda/2; LR = lamda/2;                 % Antenna lengths (lamda/2 dipoles) 
alpha_s = 5*lamda;                        % Scatterers' average cross section radius

% Dipoles - coordinates 
for i = 1:NT x_T(i) = 0; y_T(i) = (i-1)*dT;  z_T(i) = 0; end
for i = 1:NR x_R(i) = 0; y_R(i) = DD + NT*dT + (i-1)*dR; z_R(i) = 0; end

% Scatterers - box coordinates
Rx = DD/4; Ry = 3*lamda/4; Rz = Rx;         % Dimensions of the box that contains uniformly distributed scatterers
Cx = 0; Cy = -lamda; Cz = 0;                   % Center of channel (scatterers') box
...
for ic = 1:Nc
    ...
    for i=1:NR for j=1:NT
        ...
        D(i,j) = 1j*60*( exp(-1j*k0*r)/r )*( (cos(k0*LT*cos_thetaT/2) - cos(k0*LT/2))/sin_thetaT )*(2/k0)*( (cos(k0*LR*cos_thetaR/2) - cos(k0*LR/2))/sin_thetaR );
    end; end; 
    % Calculate T-matrix
    for k=1:Ns for j=1:NT
        ...
        T(k,j) = 1j*60*( exp(-1j*k0*r)/r )*( (cos(k0*LT*cos_thetaT/2) - cos(k0*LT/2))/sin_thetaT );
    end; end; 
    % Calculate R-matrix
    for i=1:NR for k=1:Ns
        ...
        R(i,k) = f*( exp(-1j*k0*r)/r )*(2/k0)*( (cos(k0*LR*cos_thetaR/2) - cos(k0*LR/2))/sin_thetaR );
    end; end; 
    %H1 = D + R*T;
    H1 = D ;
    %H1 = R*T;
